...
int main() {
  ...
  cout << (a + b) << endl; // Prints "2.3[km]". Note: units are determined by
                           // first number (a).
  cout << (b - a) << endl; // Prints "-1700[m]". Note: units are determined by
                           // first number (b).
  ...
  try {
    ...
    cout << ex.what() << endl; // Prints "Units do not match - [m] cannot be
                               // converted to [kg]"
  }
